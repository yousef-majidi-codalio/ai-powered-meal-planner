...
export default function Recipes() {
    ...
    const [applied, setApplied] = useState({ q: "", cuisine: "all", quick: true, vegan: false, time: 30 });
    const srcs = [
    ...
    const titles = ["Pasta primavera","Chicken salad","Avocado toast","Grilled salmon","Veggie bowl","Tofu stir fry","Turkey chili","Smoothie bowl","Tomato soup","Sushi bowl","Quinoa salad","Shrimp tacos","Mediterranean wrap","Pesto pasta","Veggie curry","Baked ziti","Greek salad","Miso noodles","Lentil stew","Caprese panini"];
    const mk = (i: number) => ({ title: titles[i % titles.length], image: srcs[i % srcs.length], time: `${15 + (i % 6) * 5} min`, tags: ["quick","healthy"] });
    ...
    const loadMore = () => setVisible((v) => Math.min(items.length, v + 3));
  return (
   <div className="bg-background text-foreground w-full min-w-0 overflow-x-hidden">
      ...
      <Input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="e.g. pasta" />
      </div>
      <div className="grid gap-2">
      ...
      <div className="grid gap-2">
      <label className="text-sm">Quick & Vegan</label>
      <div className="flex items-center gap-4 text-sm">
      <label className="flex items-center gap-2"><Checkbox checked={quick} onCheckedChange={(c)=>setQuick(Boolean(c))}/>Quick</label>
      <label className="flex items-center gap-2"><Checkbox checked={vegan} onCheckedChange={(c)=>setVegan(Boolean(c))}/>Vegan</label>
      </div>
      </div>
      <div className="grid gap-2">
      <label className="text-sm">Max time: {time} min</label>
      <Slider value={[time]} min={10} max={120} step={5} onValueChange={(v)=>setTime(v[0])} />
      </div>
      </div>
      <div>
      <Button variant="secondary" onClick={()=>setApplied({ q, cuisine, quick, vegan, time })}>Apply filters</Button>
      <div className="text-xs text-muted-foreground mt-1">Filters are applied locally in list previews.</div>
      </div>
      </section>
