...
export default function RecipesDiscover() {
    ...
    const titles = ["For you #1","For you #2","For you #3","For you #4","For you #5","For you #6","For you #7","For you #8","For you #9","For you #10"];
    const mk = (i: number) => ({ title: titles[i % titles.length], image: srcs[i % srcs.length], time: `${10 + (i % 8) * 5} min`, tags: ["ai","suggested"] });
    const [items, setItems] = useState(Array.from({length: 9}).map((_,i)=>mk(i)));
    const more = () => setItems((arr) => arr.concat(Array.from({length: 3}).map((_,i)=>mk(arr.length + i))));
  return (
   <div className="bg-background text-foreground w-full min-w-0 overflow-x-hidden">
      ...
      <Button onClick={more}>More like these</Button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {items.map((it, i) => (
      <RecipeCard key={`${it.title}-${i}`} title={it.title} image={it.image} time={it.time} tags={it.tags} />
      ))}
      </div>
      </section>
      </div>
