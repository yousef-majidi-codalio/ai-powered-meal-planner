...
import { Avatar, Breadcrumbs, Button, Card, CardBody, CardHeader, Chip, Image, Tab, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Tabs } from "@heroui/react";
...
export default function MyRecipes() {
    ...
    const collections = [
      { id: 1, name: "Weeknight wins", count: 14 },
      { id: 2, name: "Veggie vibes", count: 9 },
      { id: 3, name: "High protein", count: 12 }
    ];
  return (
   <div className="bg-[var(--color-background)] text-[var(--color-text)]">
    ...
    <span className="text-[var(--color-text)]/80">My recipes</span>
    </Breadcrumbs>
    <h1 className="text-2xl font-bold text-[var(--color-text)]">Collections & favorites</h1>
    </div>
    <Button className="rounded-xl bg-[var(--color-primary)] text-[var(--color-text)]">New collection</Button>
    </div>

    <Card className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background)]/60">
    <CardBody>
    <Tabs selectedKey={tab} onSelectionChange={(k) => setTab(k?.toString?.() || "favorites")} classNames={{ tabList: "bg-[var(--color-surface)]/60 rounded-xl p-1", cursor: "bg-[var(--color-primary)] rounded-lg", tab: "rounded-lg text-[var(--color-text)]" }}>
    <Tab key="favorites" title={<span className="text-[var(--color-text)]">Favorites</span>} />
    <Tab key="collections" title={<span className="text-[var(--color-text)]">Collections</span>} />
    </Tabs>
    </CardBody>
    </Card>
    </section>
  <section>
      ...
      <span className="text-[var(--color-text)] text-sm font-semibold">Favorite #{idx + 1}</span>
      <span className="text-[var(--color-text)]/70 text-xs">20 min • 350 kcal</span>
      </div>
      </div>
      <Chip className="rounded-xl bg-[var(--color-accent)]/15 text-[var(--color-accent)]">Saved</Chip>
      </CardHeader>
      <CardBody className="space-y-3">
      <Image src={src} alt={`Favorite ${idx+1}`} className="w-full h-36 object-cover rounded-xl" removeWrapper />
      <Button className="rounded-xl bg-[var(--color-primary)] text-[var(--color-text)]">Open</Button>
      </CardBody>
      </Card>
      ))}
      </div>
      ...
      <Card className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background)]/60">
      <CardHeader>
      <h3 className="text-[var(--color-text)] font-semibold">Collections</h3>
      </CardHeader>
      <CardBody>
      <Table aria-label="Collections" className="bg-transparent">
      <TableHeader>
      ...
      <TableBody>
      {collections.map((c) => (
      <TableRow key={c.id} className="hover:bg-[var(--color-surface)]/40">
      <TableCell className="text-[var(--color-text)]">{c.name}</TableCell>
      <TableCell className="text-[var(--color-text)]">{c.count}</TableCell>
      </TableRow>
      ))}
      </TableBody>
      </Table>
